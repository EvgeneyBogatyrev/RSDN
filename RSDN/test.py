...
parser.add_argument('--test_dir',type=str,default='/main')
...
parser.add_argument('--pretrain', type=str, default='/model/RSDN/RSDN.pth')
parser.add_argument('--image_out', type=str, default='/model/result')
...
def main():
    ...
    #writer = SummaryWriter()
        ...
        count = 0
        out = []
        test_list = [opt.file_test_list]
        for test_name in test_list:
            test_set = get_test_set(opt.test_dir, opt.scale, test_name)
...
def save_img(prediction,test_name,image_num, att):
    cv2.imwrite(opt.image_out + "/frame" + str(image_num + 1).zfill(4) + ".png", prediction * 255, [cv2.IMWRITE_PNG_COMPRESSION, 0])
    return
    if att == True:
