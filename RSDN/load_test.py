...
def load_img(image_path, scale, L, image_pad):
    ...
    for img_num in range(L):
        ...
        GT_temp = Image.open(image).convert('RGB')
        HR.append(GT_temp)
    return HR, len(HR)
...
class DataloadFromFolderTest(data.Dataset): # load test dataset
    def __init__(self, image_dir, scale, test_name, transform):
        ...
        alist = [line.rstrip() for line in open(os.path.join("",test_name))] 
    ...
    def __getitem__(self, index):
        ...
        LR_S = F.interpolate(LR, scale_factor=0.5, mode='bilinear', align_corners=True)
        LR_S = F.interpolate(LR_S, scale_factor=2, mode='bilinear', align_corners=True)
        LR_D = LR - LR_S

        GT_S = F.interpolate(GT, scale_factor=0.5, mode='bilinear', align_corners=True)
        GT_S = F.interpolate(GT_S, scale_factor=2, mode='bilinear', align_corners=True)
        GT_D = GT - GT_S
